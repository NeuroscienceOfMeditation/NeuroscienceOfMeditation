...
(function (root) {
  ...
  var CONCEPTS = [
    ...
    { terms: ['pranayama', 'breath', 'practice', 'technique', 'exercise', 'anxiety',
  ...
  function clean(t) {
    return String(t || '')
      .replace(/\*\*\[([^\]]+)\]\*\*/g, '[$1]')   // keep grade markers readable
      .replace(/\*\*/g, '')
  ...
  /** Pull the sentences from a passage that actually contain the query terms. */
  function bestPassage(text, terms, maxSentences) {
    ...
    if (!picked.length) return sentences.slice(0, 2).join(' ').trim();
    return picked.map(function (x) { return x.s; }).join(' ');
  }
